...
export default function LessonCard({ lesson }: LessonCardProps) {
  ...
  return (
    <Link
      ...
      className="flex items-center gap-3 px-4 h-15.5 active:bg-[#F2F2F7] transition-colors"
    >
      {/* Lesson number badge */}
      <div className="w-8 h-8 rounded-lg bg-[#F2F2F7] flex items-center justify-center shrink-0">
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-3 text-[#6C6C70]">{lesson.sentences.length} sentences</p>
      </div>
      ...
      {hasTried && (
        <span className="text-[11px] font-bold text-[#FF9500] bg-[#FF9500]/10 px-1.5 py-0.5 rounded-full shrink-0">
          {highScore}%
        </span>
      )}

      {/* Chevron */}
      <svg width="7" height="12" viewBox="0 0 8 14" fill="none" className="text-[#C7C7CC] shrink-0">
